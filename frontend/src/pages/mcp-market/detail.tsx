...
import { createOrUpdateMcpServer, deleteMcpServer, getMcpServer } from '@/services/mcpMarket';
  ...
  return (
    <PageContainer
      header={{
        ...
        onBack: () => navigate('/mcp-market/list'),
        extra: (
          <Space>
                  ...
                  <Descriptions column={2}>
                    ...
                    {authEnabled && (
                      <Descriptions.Item label={t('misc.authType')}>
                        <Space>
                          <span>
                            {(Object.keys(CredentialType).map(k => (CredentialType as any)[k]).find(ct => ct.key === authType)
