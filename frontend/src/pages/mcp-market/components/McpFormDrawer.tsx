...
import { getMcpServer } from '@/services/mcpMarket';
      ...
      if (mode === 'create') {
        ...
        if (record?.type === SERVICE_TYPE.DB && record.dbConfig) {
          ...
          if (record.dbConfig.otherParams && typeof record.dbConfig.otherParams === 'object') {
            const { otherParams } = record.dbConfig;
            const otherParamsStr = Object.keys(otherParams)
    ...
    >
      <Form form={form} layout="vertical" onFinish={handleFinish}>
        ...
        >
          <Select disabled>
            {
              Object.keys(CredentialType).map(key => (CredentialType as any)[key])
                .filter(ct => !!ct.enabled).map(ct => (
