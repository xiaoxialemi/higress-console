...
import { createOrUpdateMcpServer, listMcpServers, deleteMcpServer } from '@/services/mcpMarket';
  ...
  const columns = [
    {
      ...
      dataIndex: 'name',
      key: 'name',
      render: (text: string) => <a onClick={() => history?.push(`/mcp-market/detail?name=${text}`)}>{text}</a>,
    },
    {
    ...
    },
    {
      ...
      render: (text: any, record: any) => (
        <Space size="small">
          <a onClick={() => history?.push(`/mcp-market/detail?name=${record.name}`)}>{t('misc.detail') || '详情'}</a>
